...
| ✅ | Login | `login.html` | Auth via `Api.login()` (contrato v1 com o Pefaz). Redireciona pra `?next=` ou `/index.html` após sucesso. |
| ✅ | Cadastro | `cadastro.html` | Cria conta via `Api.signup()`. Cria student + lead no CRM na hora (mesmo sem compra). Auto-login após sucesso. |
| ✅ | Esqueci minha senha | `esqueci-senha.html` | Envia link via `Api.forgot()`. Resposta genérica pra não vazar existência de conta. |
| ✅ | Meu carrinho | `carrinho.html` | Lista items via `Api.cartList()`, permite mudar qty, aplicar cupom, remover. Checkout chama `Api.cartCheckout()` que retorna `payment_url` (Mercado Pago). Vazio mostra CTA de voltar ao catálogo. |
| ✅ | Minhas compras | `minhas-compras.html` | Lista pedidos via `Api.myOrders()`. Badge de status (pending/paid/cancelled/refunded), botão "retomar pagamento" nos pending. Deslogado mostra CTA de login com `?next=`. |
...
12. **Ecommerce Fase 2 — carrinho + minhas compras + login/cadastro** (2026-07-02). Site allaser deixa de mandar pro checkout Odoo e passa a consumir o backend do Pefaz via APIs em `cursos.allaser.com.br` (subdomínio já apontado pro VPS Pefaz). Cookies de sessão com `Domain=.allaser.com.br` — login setado no Pefaz vale automático no site allaser. **Contrato de API v1 travado** em 2026-07-02 com o dev do Pefaz — está em `assets/api.js` (Cliente + Mock). Enquanto backend não sobe, `Api` guarda tudo em `localStorage` e simula respostas com o mesmo shape. Quando `cursos.allaser.com.br/api/health` responder 200, o mock desliga automático. Front NUNCA manda preço — só slug/qty/cupom. Um checkout gera UM pagamento agregado (Mercado Pago), backend matricula todos os itens quando o webhook confirma pago.
