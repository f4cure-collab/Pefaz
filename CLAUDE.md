...
> **URLs limpas (sem `.html`)**: desde 2026-07-28 o site usa `.htaccess` com rewrite pra URLs limpas. `/blog.html` redireciona 301 pra `/blog`, e `/blog` serve `/blog.html` internamente. **Sempre linkar sem `.html`** (`href="/blog"`, não `href="/blog.html"`). Regras estão no `.htaccess` da raiz. LiteSpeed/Apache aplicam em produção — `php -S` local não aplica (usa `.html` direto pra testar).
>
> **Cassia Fukuda NUNCA aparece no site**: saiu da empresa (2026-07-28). Nem nome, nem foto. Se encontrar refs históricas em posts do blog ou LPs, remover na mesma passada.
...
| ✅ | Blog (index + posts) | `blog.html` + `blog/` | Estático, gerado por `scripts/generate-blog.js`. **Fonte de dados mudou 2026-08-14**: agora consome `cursos.allaser.com.br/api/site-blog.php?slug=allaser&full=1` (backend Allaser) em vez do WP-JSON (WP foi desligado). Retorna 85 artigos (78 com imagem) em uma chamada com `content_html`. Encoding UTF-8 correto. Regera com `node scripts/generate-blog.js --all`. Ficheiros órfãos: 8 posts antigos no repo que não estão mais na API — regeração não deleta (só sobrescreve o que existe). |
| ✅ | Categorias do blog | filtro local no `blog.html?cat=<slug>` | Reaproveita o filtro por `data-cat` que já existia. Adicionado `data-cat-slug` em cada botão + suporte a `?cat=<slug>` na URL (JS lê no load, aplica filtro, cliques atualizam URL via `history.replaceState`). Footer aponta pra `/blog.html?cat=X` — não gera páginas separadas por categoria. |
...
| ✅ | LP Start Laser Presencial POA | `lps/cursos/start-laser-poa.html` | Criada 2026-07-28. Presencial em Porto Alegre com a Profa. Dra. Daiane Meneguzzo (foto Daiane sozinha, do WP `Design-sem-nome-16-e1764021800319.png` → `daiane.webp`). CTA em 6 pontos abre **modal Kommo** (nome/email/telefone) → posta em `/lps/cursos/submit-lead.php` → redireciona pro Eduzz `chk.eduzz.com/39YN53ADWO`. 4 lotes com bônus: Lote 1 (5v, livro ILIB) · Lote 2 (15v, hands on 26/09-11/11-11/12) · Lote 3 (20v, masterclass à escolha) · Lote 4 (sem bônus). Assets em `/assets/images/lps/start-laser-poa/` — autônoma do WP. |
| ✅ | LPs de curso replicadas do WP | `lps/cursos/*.html` + `lps/masterclasses/*.html` | **Reescritas do zero em 2026-07-05** — HTML/CSS enxuto (~18KB cada, era 1.5MB no clone Elementor). 4 iterações: (1) reescrita base; (2) ajustes manuais; (3) workflow multiperspectiva 3 lentes (27 análises → 108 findings → 52 fixes paralelos); (4) workflow adversarial 3 juízes céticos + fixes manuais nas 2 mais fracas. Scores adversariais finais (média 65/100): rinite 82, os outros 72, fotoneuro/pos-parto corrigidos manualmente pós-score. Cada LP com paleta e layout próprios: laserpuntura (branco/verde vibrante + estátua + 3 fotos profs reais), start-laser (dark + lime + Daiane + glow), feridas (creme + hero mão + 4 profs), fisioterapia (branco/lime + foto joelho fisio2.png), fotoneuromodulacao (dark neuro + bullets sobre card), pos-parto (branco + banner real das alunas), especialidades-medicas (dark + dourado + 2 fotos médicos + depoimentos), rinite (dark + vermelho + Daiane + instrutora), bronquiolite (dark + pulmões + oliva + benefícios). Todas com GTM/Pixel, modal submit-lead, redirect Odoo. Refs WP em `_wp_refs/*.png`, prod em `_prod_shots/*.png`. |
| ✅ | Política de Privacidade | `politica-de-privacidade.html` | LGPD completa: dados coletados, uso, compartilhamento, cookies, direitos do titular, retenção, segurança. Contato: contato@allaser.com.br. |
...
12. **Ecommerce — front do catálogo + carrinho** (contrato v2, 2026-07-02). Backend já no ar em `cursos.allaser.com.br` (subdomínio aponta pro VPS Pefaz). **Escopo do site allaser: SÓ catálogo + carrinho + botão "Finalizar compra"**. Login, cadastro, checkout, pagamento, matrícula, minhas compras — TUDO servido pelo backend em `cursos.allaser.com.br`. O site allaser só linka pra lá. Regras invioláveis do contrato: (a) TODO fetch com `credentials:'include'` — cookies com `Domain=.allaser.com.br` são compartilhados; (b) Front NUNCA calcula nem manda preço — só `product_slug`, `offer_id?` e `qty`. Preços vêm de `/api/catalog.php` e são sincronizados nos cards via `Api.syncPrices()`; (c) Todo POST precisa do header `X-CSRF` — token vem de `/api/me.php` no boot e fica guardado no `Api`; (d) NÃO disparar `fbq('track','Purchase')` — o backend faz isso na página de checkout dele. Endpoints em uso: `GET /api/me.php`, `GET /api/catalog.php`, `GET /api/cart-list.php`, `POST /api/cart-add.php`, `POST /api/cart-update.php` (qty 0 = remove), `POST /api/cart-remove.php`. Cliente e URLs do backend em `assets/api.js` (`Api.URLS.checkout|login|logout|account|myOrders`).

13. **Allaser News — revista + cadastro** (2026-09-16). Revista em `/allasernews/` (ver `allasernews/README.md` para publicar uma edição). O formulário "Receba a Allaser News" fala com o backend por `Api.newsSubscribe()` → **`POST /api/news-subscribe.php`**, endpoint próprio: não é `store-lead.php` (aquele cria carrinho + prefill de checkout) nem `free-enroll-request.php` (cria conta e manda link de senha). Campos: `name`, `email`, `phone` (opcional, E.164 fora do BR), `consent`, `source:"allasernews"`, `latest_edition`, `pagina`, UTMs e `ind`. Resposta esperada `{ok:true}` ou `{ok:false,error}`. Endpoint no ar desde 2026-09-17. Sucesso é só `{ok:true}`; `422` devolve mensagem de validação pronta pra pessoa ler e o resto vira "tente de novo". Falha de sistema (não de validação) registra a tentativa no `track.php` como `news_subscribe_falhou` com todos os campos, pra o contato não sumir e dar pra recuperar à mão — **isso não vira sucesso na tela**: prometer "cadastro confirmado" sem o backend ter aceitado deixaria a pessoa esperando uma revista que nunca chega. **O `latest_edition` sai do `href` do primeiro cartão de edição**, porque o backend monta o link do e-mail como `https://allaser.com.br/allasernews/<latest_edition>` — cartão fora de ordem manda e-mail pra edição errada. O **mesmo cadastro está no rodapé** (`wireFooterNews` em `components.js`), presente em todas as páginas que usam o footer compartilhado — 8 institucionais + 88 do blog; as LPs de tráfego pago têm footer próprio e não entram. Lá não há cartão pra ler, então o slug vem da constante `NEWS_LATEST_EDITION`, que **precisa ser atualizada a cada edição nova** junto com o cartão. O rodapé pede nome + e-mail + aceite porque o backend recusa sem os três; o campo único de e-mail que existia antes nunca teria funcionado. Quando o endpoint existir, o caminho normal assume sozinho, sem tocar no front. Comportamento em `assets/allasernews/inscricao.js`; o HTML nasce com o botão desabilitado de propósito, e é o script que o libera.
14. **Menu do header** (2026-09-16). Ordem: **Cursos · Quem Somos · Sobre o Laser · Allaser News · Blog · Encontre um Profissional**. "Cursos" (antigo "Home") aponta pra `/#cursos-home` — a própria home, já rolada até o catálogo, pulando a hero. Por isso `#cursos-home` ganhou `scroll-margin-top` no `index.html` (o header sticky cobriria o título) e `shared.js` fecha o painel mobile ao clicar em qualquer `.mnav__link` (âncora na mesma página não recarrega, e o menu ficaria aberto com o scroll travado). A home chama `setActiveNav('cursos')`.
...
- **`cursos-data.js` tem 31 entradas** — todos com capa 9:16 em `/assets/images/capas-cursos/`. Histórico: Laserpuntura removido 2026-07-23; Plantão de Dúvidas removido 2026-07-24 (produto descontinuado); 3 eventos gravados adicionados 2026-07-24 (v-forum-2025, iv-forum-2024, ilib-summit-2023) com capas placeholder geradas — trocar quando tiver capa real.
- **Cursos com checkout novo (via `cursos.allaser.com.br/checkout?add=<slug>`)**: fisioterapia, hands-on-dosimetria, v-forum-2025, iv-forum-2024, ilib-summit-2023. Os demais continuam apontando pro Odoo antigo em `allasercursos.odoo.com/shop/...` — regra oficial: **a lista de checkout válido sempre sai de `https://cursos.allaser.com.br/api/catalog.php`** (só linkar o que aparece lá). Se adicionar curso novo no site, confirmar antes se ele está no catalog.php. Cursos sem capa (presenciais com data específica, fóruns gravados, materiais avulsos, plantão start-laser, combos ocultos, avançados) foram removidos em 2026-06-26 quando padronizamos o pôster 9:16. Se precisar reativar algum, providencie a capa antes de re-adicionar no `cursos-data.js`.
...
- **Imagens do blog quebradas por bug de migração histórico**: 82 arquivos em `assets/images/blog/` eram HTML disfarçado com extensão `.jpg`/`.png`/`.jpeg` — o script que migrou do WP em julho salvou páginas HTML de erro do WordPress quando a imagem original não existia. Fix aplicado 2026-07-29: refs removidas dos posts (29 posts limpos) + arquivos deletados. **Se for baixar imagens do WP novamente, validar magic number** (`FF D8` JPEG, `89 50 4E 47` PNG, `47 49 46` GIF, `RIFF...WEBP`) antes de gravar — content-type do HTTP não é confiável, o WP retorna 200 com HTML mesmo pra 404.
