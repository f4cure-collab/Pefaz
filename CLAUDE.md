...
13. **Allaser News — revista + cadastro** (2026-09-16). Revista em `/allasernews/` (ver `allasernews/README.md` para publicar uma edição). O formulário "Receba a Allaser News" fala com o backend por `Api.newsSubscribe()` → **`POST /api/news-subscribe.php`**, endpoint próprio: não é `store-lead.php` (aquele cria carrinho + prefill de checkout) nem `free-enroll-request.php` (cria conta e manda link de senha). Campos: `name`, `email`, `phone` (opcional, E.164 fora do BR), `consent`, `source:"allasernews"`, `latest_edition`, `pagina`, UTMs e `ind`. Resposta esperada `{ok:true}` ou `{ok:false,error}`. **Enquanto o backend não publicar o endpoint** (hoje responde 404), nada se perde: `Api.newsSubscribe` cai no `track.php` (evento `news_subscribe`, mesmos campos) e só devolve `ok` depois que ELE confirma o recebimento. Isso importa porque o POST leva `Content-Type` JSON + `X-CSRF`, então o browser faz preflight `OPTIONS` — e num endpoint inexistente o preflight responde 404 **sem cabeçalho CORS**, o browser barra a resposta e o `fetch` rejeita sem nunca expor o status. Falha de rede real cai no mesmo lugar; quem separa os dois casos é o `track.php`, que tem CORS e responde 204. Quando o endpoint existir, o caminho normal assume sozinho, sem tocar no front. Comportamento em `assets/allasernews/inscricao.js`; o HTML nasce com o botão desabilitado de propósito, e é o script que o libera.
