...
| ✅ | LPs de curso replicadas do WP | `lps/cursos/*.html` + `lps/masterclasses/*.html` | **Reescritas do zero em 2026-07-05** — HTML/CSS enxuto (~18KB cada, era 1.5MB no clone Elementor). Cada LP com paleta e layout próprios imitando o visual da versão WP correspondente: laserpuntura (branco/verde-oliva + estátua), start-laser (dark + lime + foto Daiane), feridas (hero azul-claro + foto mão sendo tratada), fisioterapia (branco/lime + seção dark), fotoneuromodulacao (dark neuro), pos-parto (branco + banner dark), especialidades-medicas (dark + laranja + lime), rinite (dark + vermelho), bronquiolite (dark + pulmões + oliva). Todas incluem GTM/Pixel, modal com submit-lead, redirect pro Odoo. Referências WP em `_wp_refs/*.png`, screenshots das minhas em `_local_shots/*.png` para iteração visual. |
