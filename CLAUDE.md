...
| ✅ | LPs de curso replicadas do WP | `lps/cursos/*.html` + `lps/masterclasses/*.html` | Em 2026-07-04, 9 LPs replicadas do WordPress via workflow (fetch + build em paralelo): start-laser, laserpuntura, feridas, fisioterapia, fotoneuromodulacao, pos-parto, especialidades-medicas + masterclasses rinite e bronquiolite. Cada uma segue o padrão visual das já existentes (`saude-da-mulher.html`, `fbm-estetica-facial.html`, `fotobiomodulacao-na-oncologia.html`) — hero dark, tópicos, professora Daiane, benefícios, CTA + modal com submit-lead. `cursos-data.js` reapontado pras versões locais. |
